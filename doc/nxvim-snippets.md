...
list `prefix` fans out into one snippet per trigger. A `package.json` on the runtimepath that is not
a snippet collection is skipped silently; a snippet file that is unreadable or malformed is reported
with `nx.notify` and skipped, so one bad file does not sink a whole language.
...
Resolved at expand time, from the buffer and the clock:
...
- A multi-line body is re-indented: every line after the first is
  prefixed with the anchor line's leading whitespace.
- Jumping past the last stop ends the session, as does `snip.abort()` or
  a wholesale buffer reload (undo/redo, `:e`), where the anchors are
  moot.
...
by the snippet's `description` when it has one.
...
auto-joins `nx.complete`) and install the jump keymaps.
...
- `snip.add_collection(dirs)` — add a VSCode collection root (a dir, or
  ...
  not on the runtimepath. Call it at config time, before completions
  start, so already-loaded filetypes see it too.
...
Against the full friendly-snippets collection (9,422 snippets across 127 filetypes): all 9,422
parse, and 9,420 (99.98%) lay out and expand. The 2 that do not use a regex lookbehind
